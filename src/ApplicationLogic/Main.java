...
//        calendar.buyActivity(activityPeriodic);
        calendar.putSegment(calendar.getDayByDate(LocalDate.now()), calendar.getActivitiesInCalendar().get("Wynieś dzieci").getNextSegment(50400));
        calendar.putSegment(calendar.getDayByDate(LocalDate.now()), calendar.getActivitiesInCalendar().get("Bądź prezydentem").getNextSegment(50500));
        calendar.putSegment(calendar.getDayByDate(LocalDate.now().plusDays(1)), calendar.getActivitiesInCalendar().get("Zabij kolege").getNextSegment(9_000));

        //calendar.getUser().getDays().get(0).print();

        new UsersManager().saveConfiguration(calendar.getUser());
    }
}
